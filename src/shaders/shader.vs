...
void paint(out vec4 vColor, vec2 point, float time_ms, vec2 touchPoint) {
...
void main() {
  ...
  vec2 touchPoint = mapToClipSpace(touchPoint, resolution);

  gl_Position = vec4(point.xy, 0, 1);
  gl_PointSize = 5.0;

  paint(vColor, point, time, touchPoint);
}